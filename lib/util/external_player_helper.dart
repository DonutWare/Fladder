...
class ExternalPlayerHelper {
  ...
  static Future<void> launchEnergyPlayer(WidgetRef ref, ItemBaseModel item) async {
    final user = ref.read(userProvider);
    if (user == null) return;

    final serverUrl = user.credentials.url;
    final apiKey = user.credentials.token;

    if (serverUrl.isEmpty || apiKey.isEmpty) return;

    final videoId = item.id;
    final streamUrl = "$serverUrl/Videos/$videoId/stream?static=true&api_key=$apiKey";

    // Subtitles
    final currentSub = item.streamModel?.currentSubStream;
    final subsPath = (currentSub != null && currentSub.url != null) ? currentSub.url! : "";

    // Start position (Energy Player uses seconds)
    final startTime = (item.userData.playbackPositionTicks) ~/ 10000000;

    final energyUrl = Uri.encodeFull(
      'energyplayer:launch=media&path="$streamUrl"&subsPath="$subsPath"&startTime=$startTime',
    );
